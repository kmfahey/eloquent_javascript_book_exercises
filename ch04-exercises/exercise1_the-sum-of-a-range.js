...
function range(start, end, step) {
    ...
    if (step > 0 && start > end) {
        throw new Error("step is positive and start is greater than end");
    } else if (step < 0 && end > start) {
        throw new Error("step is negative and end is greater than start");
    } else if (step == 0) {
...
function sum(start, end) {
    let retval = 0;
    for (let operand of range(start, end)) {
        retval += operand;
    }
    return retval;
}
