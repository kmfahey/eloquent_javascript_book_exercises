 ...
 * [N]. The returned value will be a string containing only one character (for
...
function countChar(strval, charSeeking) {
    let charCount = 0;
    for (i = 0; i < strval.length; i++) {
        if (strval[i] === charSeeking) {
            charCount++;
        }
    }
    return charCount;
